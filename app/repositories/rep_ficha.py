...
def obtener_ficha(db: Session, cliente_id: str) -> dict | None:
    ...
    cli = db.execute(
        text("""
            SELECT 
                c.*,
                (SELECT estado FROM solicitudes_credito WHERE cliente_id = c.id ORDER BY created_at DESC LIMIT 1) AS ultima_solicitud_estado,
                (SELECT calificacion_sbs FROM consultas_buro WHERE cliente_id = c.id ORDER BY created_at DESC LIMIT 1) AS ultimo_buro_sbs
            FROM clientes c 
            WHERE c.id = :id
        """), {"id": cliente_id}
    ).mappings().first()
    ...
    # Obtener el ultimo resultado de visita
    visita = db.execute(
        text("SELECT estado_visita, resultado_visita FROM cartera_diaria WHERE cliente_id = :id ORDER BY fecha_asignacion DESC LIMIT 1"),
        {"id": cliente_id}
    ).mappings().first()

    # Determinar calificacion real
    if cli["ultima_solicitud_estado"] == 'rechazado':
        riesgo_final = "RECHAZADO"
    elif cli["ultimo_buro_sbs"]:
        riesgo_final = cli["ultimo_buro_sbs"]
    else:
