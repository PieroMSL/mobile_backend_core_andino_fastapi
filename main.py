...
async def log_requests(request: Request, call_next):
    if "/cliente" in request.url.path:
        print(f"--- NUEVA PETICION A {request.url.path} ---")
        print(f"Headers recibidos en FastAPI:")
        for k, v in request.headers.items():
            print(f"  {k}: {v}")
        print(f"Authorization recibido: {request.headers.get('Authorization', 'None')}")
        print("---------------------------------------")
